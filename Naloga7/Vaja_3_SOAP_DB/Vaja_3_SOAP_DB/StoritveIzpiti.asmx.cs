...
namespace Vaja_3_SOAP_DB
{
    ...
    public class StoritveIzpiti : System.Web.Services.WebService
    {
        ...
        {
            return Podatki.OdstraniOsebo(id);
        }
        [WebMethod]
        public int OdstraniPodjetje(int id)
        {
            return Podatki.OdstraniPodjetje(id);
        }
        [WebMethod]
        public int OdstraniOseboIzPodjetjaByIndex(int indexOseba, int indexPodjetej)
        {
            return Podatki.RemoveOsebeIzPodjetjaByIndex(indexOseba, indexPodjetej);
        }
        [WebMethod]
        public int AddOsebaVPodjetjeByIndex(int indexO, int indexP, string delovnoMesto, int letoDo)
        {
            return Podatki.AddOsebaVPodjetjeByIndex(indexO, indexP, delovnoMesto, letoDo);
        }
    }
}
